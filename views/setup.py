...
def render(hh: Household) -> None:
    ...
    st.title("⚙️ Setup")
    st.caption(
        "Configure your household numbers, sync live portfolio data from FinExtract, "
        "and manage your data bridge for the public site."
    )
    ...
    if _sync:
        ...
        if snap.server_available:
            ...
            save_snapshot(snap)
            # Push synced balance into session state
    ...
    # ------------------------------------------------------------------
    # Section 4: Data bridge
    # ------------------------------------------------------------------
    st.header("Data bridge")
    _handle_v2_privkey()
