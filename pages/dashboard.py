...
from engine.scenario import run_scenario, run_no_conversion, auto_fill_12, ConversionPlan


def render(hh: Household):
    st.title("📊 Dashboard — Is Converting Worth It?")
    st.caption(f"You {hh.your_age} / Spouse {hh.spouse_age} · "
    ...
    years = [yr.year for yr in no_conv.years]
    ...
    ct_nc, ct_wc = 0, 0
    for t_nc, t_wc in zip(tax_nc, tax_wc):
        ct_nc += t_nc; ct_wc += t_wc
        cum_tax_nc.append(ct_nc); cum_tax_wc.append(ct_wc)

    # Brokerage balance
    brok_nc = [yr.brokerage_balance for yr in no_conv.years]
    brok_wc = [yr.brokerage_balance for yr in with_conv.years]
    ...
    with col4:
        ...
        st.metric("Net Lifetime Benefit", f"${net:,.0f}",
    ...
    fig_ira.add_trace(go.Scatter(
        x=ages, y=ira_nc, name="No Conversion",
        line=dict(color="#ef4444", width=2, dash="dash"),
        hovertemplate="Age %{x}<br>IRA: $%{y:,.0f}<extra>No Conversion</extra>"
    ))
    fig_ira.add_trace(go.Scatter(
        x=ages, y=ira_wc, name="With Conversion (12%)",
    ...
    fig_ira.update_layout(
        ...
        xaxis_title="Your Age", yaxis_title="IRA Balance ($)",
        yaxis_tickformat="$,.0s",
    ...
    with col_right:
        ...
        fig_cum.add_trace(go.Scatter(
            x=ages, y=cum_tax_nc, name="No Conversion",
            fill="tozeroy", fillcolor="rgba(239,68,68,0.15)",
            line=dict(color="#ef4444", width=2),
        ))
        fig_cum.add_trace(go.Scatter(
            x=ages, y=cum_tax_wc, name="With Conversion",
            fill="tozeroy", fillcolor="rgba(59,130,246,0.15)",
            line=dict(color="#3b82f6", width=2),
        ))
        fig_cum.update_layout(
            title="Cumulative Federal Tax",
            xaxis_title="Your Age", yaxis_title="Cumulative Tax ($)",
            yaxis_tickformat="$,.0s",
    ...
    cum_rmd_savings = 0
    cum_brok_savings = 0
    for yr_nc, yr_wc in zip(no_conv.years, with_conv.years):
        cum_conv_tax = with_conv.total_conv_tax  # sunk cost paid during conv years
        cum_rmd_savings += yr_nc.federal_tax_amt - yr_wc.federal_tax_amt if yr_nc.your_age >= 75 else 0
    ...
    fig_net = go.Figure()
    colors = ["#ef4444" if v < 0 else "#22c55e" for v in net_benefit]
    fig_net.add_trace(go.Scatter(
        x=ages, y=net_benefit, name="Net Benefit",
        fill="tozeroy",
    ...
    # Find break-even age
    breakeven = None
    for i, (a, nb) in enumerate(zip(ages, net_benefit)):
        if nb >= 0 and a >= 75:
            breakeven = a
            break

    if breakeven:
        fig_net.add_annotation(x=breakeven, y=0, text=f"Break-even: age {breakeven}",
                              showarrow=True, arrowhead=2, bgcolor="#22c55e",
                              font=dict(color="white"))

    fig_net.update_layout(
        title="Net Benefit: Conversion Tax Paid vs RMD + Brokerage Tax Saved",
        xaxis_title="Your Age", yaxis_title="Net Benefit ($)",
        yaxis_tickformat="$,.0s",
        template="plotly_dark", height=400,
    )
    ...
    for col, (age, label) in zip(cols, milestones):
        ...
        with col:
            ...
            st.caption(label)
            if yr_nc and yr_wc:
                st.markdown(f"IRA (NC): **${(yr_nc.your_ira_begin+yr_nc.spouse_ira_begin)/1e6:.1f}M**")
                st.markdown(f"IRA (WC): **${(yr_wc.your_ira_begin+yr_wc.spouse_ira_begin)/1e6:.1f}M**")
