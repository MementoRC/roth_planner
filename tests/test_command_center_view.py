...
The ``txn_price_now``/``grants`` card tests (formerly rendered here, then
SKIPPED pending an owning partial) moved to
``tests/test_setup_options_partial.py`` as of Task 5 — that behavior now
lives in ``views/setup/_partials.py:render_options_partial``.
"""
